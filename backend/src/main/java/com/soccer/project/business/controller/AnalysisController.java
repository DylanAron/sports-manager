...
public class AnalysisController {
    ...
    public ResponseEntity<PageResp<Analysis>> selectByPage(@RequestBody PageRequest<AnalysisListReqVo> pageRequest) {
        Page<Analysis> page = new Page<>(pageRequest.getPageNum(), pageRequest.getPageSize());
        PageResp<Analysis> pageResult = analysisService.selectByPage(page, pageRequest.getData());
        return ResponseEntity.success(pageResult);
    }

    @Transactional
    @Log(title = "添加分析", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public ResponseEntity<Void> add(@RequestBody Analysis analysis) {
    ...
    @Log(title = "修改分析", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public ResponseEntity<Void> edit(@RequestBody Analysis analysis) {
        if (analysis.getId() == null) return ResponseEntity.error(RespEnum.ID_CAN_NOT_BE_NULL);
        analysisService.updateById(analysis);
        return ResponseEntity.success();
    }

    @Log(title = "删除分析", businessType = BusinessType.DELETE)
    @PostMapping("/delete")
