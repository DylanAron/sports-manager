...
@RequestMapping("/business/halffull")
@RestController
public class HalfFullController {
    @Autowired
    private HalfFullService halfFullService;

    @PostMapping("selectByPage")
    public ResponseEntity<PageResp<HalfFull>> selectByPage(@RequestBody PageRequest<HalfFullListReqVo> pageRequest) {
        Page<HalfFull> page = new Page<>(pageRequest.getPageNum(), pageRequest.getPageSize());
        PageResp<HalfFull> pageResult = halfFullService.selectByPage(page, pageRequest.getData());
        return ResponseEntity.success(pageResult);
    }

    @Transactional
    @Log(title = "添加半全场", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    public ResponseEntity<Void> add(@RequestBody HalfFull halfFull) {
        halfFullService.save(halfFull);
    ...
    @Log(title = "修改半全场", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    public ResponseEntity<Void> edit(@RequestBody HalfFull halfFull) {
        if (halfFull.getId() == null) return ResponseEntity.error(RespEnum.ID_CAN_NOT_BE_NULL);
        halfFullService.updateById(halfFull);
        return ResponseEntity.success();
    }

    @Log(title = "删除半全场", businessType = BusinessType.DELETE)
    @PostMapping("/delete")
    public ResponseEntity<Void> delete(Integer id) {
        if (id == null) return ResponseEntity.error(RespEnum.ID_CAN_NOT_BE_NULL);
        halfFullService.removeById(id);
        return ResponseEntity.success();
    }
}
