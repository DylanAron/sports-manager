<template>
	...
	<el-dialog v-model="props.isOpen" :title="props.title" min-width="620" align-center
		@close="emit('update', { reload: false })" class="single-bag-add">
			...
			<div class="content">
				<div class="detail-row" v-for="(item, index) in form.matchList">
					<div class="row-content">
						...
						<div class="row-item">
							<el-input v-model="item.serialize" autocomplete="off" placeholder="序列号(北单073)" />
						</div class="row-item">
						<div class="row-item">
...
import { add as addSingleBag, update as updateSingleBag, getMatchListByGroup } from "@/api/business/singleBag";
...
const doSubmit = () => {
	...
	if (form.id == null) {
		//添加
		addSingleBag(params).then(resp => {
			ElMessage.success("添加成功!");
			emit('update', { type: 0, reload: true })
		})
	} else {
		updateSingleBag(params).then(resp => {
