<template>
	...
	<el-dialog v-model="props.isOpen" :title="props.title" min-width="620" align-center
		@close="emit('update', { reload: false })" class="snipe-add">
		<el-form :inline="true" :model="form" label-position="right">
			<!-- <el-form-item label="总回报率" required>
				<el-input v-model="form.rateOfReturn" autocomplete="off" size="large" placeholder="100%~200%" />
			</el-form-item> -->
			<el-form-item label="赛事日期" required>
				<div><el-date-picker v-model="form.matchDate" size="large" type="date" placeholder="选择日期" :size="size"
			...
			<div class="content">
				<div class="detail-row" v-for="(item, index) in form.matchList">
					<div class="row-content">
						...
						<div class="row-item">
							<el-date-picker v-model="item.matchTime" type="datetime" placeholder="赛事时间" value-format="YYYY-MM-DD HH:mm:ss" />
						</div>
						<div class="row-item">
							<el-select v-model="item.matchStatus" placeholder="赛事状态" size="large">
...
import { add as addHalfFullSnipe, getMatchListByGroup, update as updateSnipe } from "@/api/business/halfFullSnipe";
import useConvertHitResult from "@/hooks/useConvertHitResult"
import { format, isToday,isBefore ,isAfter,startOfToday,endOfToday} from 'date-fns';
...
const doSubmit = () => {
	...
	if (form.matchDate == null || form.matchDate == '') {
		ElMessage.error("请选择狙击日期");
		return;
	}

	if (isToday(form.matchDate) && (form.payCode == null || form.payCode == '')) {
		ElMessage.error("请选择支付码");
		return;
	}
	...
	for (var i = 0; i < form.matchList.length; i++) {
		...
		if (match.serialize == null || match.serialize == '') {
			ElMessage.error("请填写第" + (i + 1) + "场赛事的序列号");
			return;
		}
	...
	if (form.id == null) {
		//添加半全场
		addHalfFullSnipe(params).then(resp => {
			ElMessage.success("添加成功!");
			emit('update', { type: 0, reload: true })
		})
	} else {
		updateSnipe(params).then(resp => {
