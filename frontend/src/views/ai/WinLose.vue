<template>
  <div class="main">
    ...
    <!-- 添加/修改弹窗 -->
    <el-dialog v-model="dialogVisible" :title="dialogType === 'add' ? '添加' : '修改'" width="650px" :close-on-click-modal="false" class="winlose-dialog" top="5vh">
      <el-form label-width="100px" v-if="form" class="winlose-form">
...
import { selectByPage, uploadImage, add, edit, del } from '@/api/business/winlose';
...
function doGetTableData() {
  ...
  if (queryDate.value) {
    params.data.matchDate = queryDate.value;
  }
  selectByPage(params).then(resp => {
